...
describe('Collection Roles', () => {
  ...
  describe('GET /v1/collections/:collectionId/roles', () => {
    it('200s with has role boolean for role and address or walletId', done => {
      chai.request(server)
        .get(`/v1/collections/${testCollection.id}/roles?address=${testPlayer.wallet.address}&role=minter`)
        .end((error, response) => {
    ...
    it('400s when not provided address or role', done => {
      chai.request(server)
        .get(`/v1/collections/${testCollection.id}/roles?address=${testPlayer.wallet.address}`)
        .end((error, response) => {
          helpers.logResponse(response);
  ...
  describe('POST /v1/collections/:collectionId/roles', () => {
    it('200s with transaction object and grants role to the provided address', done => {
      ...
      chai.request(server)
        ...
        .end((error, response) => {
          ...
          chai.request(server)
            .get(`/v1/collections/${testCollection.id}/roles?address=${testPlayer.wallet.address}&role=${fields.role}`)
            .end((error, response) => {
  ...
  describe('DELETE /v1/collections/:collectionId/roles', () => {
    it('200s with transaction object and removes role for the provided address', async () => {
      ...
      await new Promise(resolve => {
        chai.request(server)
          .delete(`/v1/collections/${testCollection.id}/roles`)
          .set('X-Authorization', testGame.secretKey)
          .set('X-Wallet-Decrypt-Key', testGame.walletDecryptKey)
          .send(fields)
          .end((error, response) => {
